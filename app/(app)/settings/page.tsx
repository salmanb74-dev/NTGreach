...
export default async function SettingsGeneralPage() {
  ...
  const dealDefaults = parseDealQuoteDefaults(
    settingsMap[DEAL_QUOTE_DEFAULTS_SETTING_KEY]
  )
  ...
  async function saveInputCurrency(formData: FormData) {
    ...
    await updateAppSetting(
      'input_currency',
      formData.get('input_currency') as string
    )
  }
    ...
    <div>
      ...
      <div className={styles.section}>
        ...
        <div className={styles.sectionDesc}>
          Input currency is used for all data entry. View currencies are available
          as display options on dashboards and reports.
        </div>
        ...
        <form action={saveViewCurrencies} className={styles.form}>
          <div className={styles.field}>
            <label className={styles.label}>
              View Currencies (select all that apply)
            </label>
              ...
              {currencies?.map(c => (
                <label key={c.value} className={styles.checkRow}>
                  <input
                    ...
                    value={c.value}
                    defaultChecked={
                      viewCurrencies.includes(c.value) || c.value === 'USD'
                    }
      ...
      <div className={styles.section}>
        ...
        <div className={styles.sectionDesc}>
          Fetches live USD cross-rates (open.er-api.com free, or{' '}
          <code>EXCHANGE_RATE_API_KEY</code> → exchangerate-api.com). If live
          fetch fails, yesterday&apos;s saved rates are copied. Last updated:{' '}
